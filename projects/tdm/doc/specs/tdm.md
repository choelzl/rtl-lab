...
| AGU[0], AGU[1]        | `tdm_agu.hpp`    | Address Generation Units (x-OBI managers). The TDM design's **own** driver. Each parses the trace's first line for the kernel-wide mapping parameters (driven to the TDM mux), then replays its CSV trace, issuing a group of `N_REQ = 4` words per cycle as one x-OBI sub-request (single base address).                                                                                                                                                                                                                                                                                                                                                               |
...
2. **Fair comparison sizing** — the mapping needs `N_BANK ≥ 32`; run the crossbar baseline at the same `N_BANK` (e.g. `PARAMS="N_BANK=32"`) for an apples-to-apples penalty comparison (the two tops currently default to `N_BANK=32` and `8` respectively).
