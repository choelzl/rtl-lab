...
| Bank[0..3]     | `bank.hpp`            | Four memory banks (OBI subordinates) holding the **bank registers** — the next register stage after the AGU. Each bank's logic converts the registered OBI request into an access to its internal memory array (a read when `we=0`, a write when `we=1`), returning `rdata`.                                                                                                                                                                                                                                                                      |
...
The path is registered at **two stages only**; the 4×4 crossbar between them is combinational routing (plus the per-bank round-robin arbiter) and adds no register of its own:
