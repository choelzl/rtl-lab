...
//
// Template parameters:
//   NUM_AGU, NUM_WAGU, NUM_REQ, NUM_BANK, NUM_ROW, BYTES_PER_WORD, WORDS_PER_ROW
// -----------------------------------------------------------------------------
...
template <int NUM_AGU = 2, int NUM_WAGU = 2, int NUM_REQ = 4, int NUM_BANK = 8,
          int NUM_ROW = 1024, int BYTES_PER_WORD = 4, int WORDS_PER_ROW = 4>
SC_MODULE(top_crossbar) {
    // -----------------------------------------------------------------------
    // Derived constants
    // -----------------------------------------------------------------------
    static constexpr int NUM_RD        = NUM_AGU  * NUM_REQ;
    ...
    static constexpr int N_L1L2_RD = NUM_AGU  * NUM_REQ;   // = NUM_RD
    ...
    sc_vector<crossbar<NUM_AGU,  NUM_BANK_GRP, 1, ROUTE_LSB + LOG_REQ, LOG_BANK_GRP>> l2_rd_;
    sc_vector<crossbar<NUM_WAGU, NUM_BANK_GRP, 1, ROUTE_LSB + LOG_REQ, LOG_BANK_GRP>> l2_wr_;
    sc_vector<crossbar<2,        2,            1, L3_SEL,              1           >> l3_;
    sc_vector<bank<NUM_ROW / 2, BYTES_PER_WORD>>                                                         banks_;
          ...
          l3_("l3"), banks_("bank")
    {
        ...
        for (int m = 0; m < NUM_RD; ++m) sensitive << m_addr_i[m];

        SC_METHOD(hash_wr_addr);
        for (int m = 0; m < NUM_WR; ++m) sensitive << m_w_addr_i[m];

        SC_METHOD(compute_bk_laddr);
        for (int i = 0; i < N_PHYS; ++i) sensitive << l3bk_addr[i];

        // ----- L1 read -------------------------------------------------------
        // L1_RD[j]: AGU j's NUM_REQ ports → NUM_REQ L2_RD groups
        for (int j = 0; j < NUM_AGU; ++j) {
