...
| `MAX_VAL_A`    | `1`..`2^(IN_WIDTH_A-1)-1` | Max positive value for `a` test inputs; values are drawn symmetrically in `[-MAX_VAL_A, +MAX_VAL_A]` (default: `2^(IN_WIDTH_A-1)-1`) |
| `MAX_VAL_B`    | `1`..`2^(IN_WIDTH_B-1)-1` | Max positive value for `b` test inputs; values are drawn symmetrically in `[-MAX_VAL_B, +MAX_VAL_B]` (default: `2^(IN_WIDTH_B-1)-1`; not available on alpha variants) |
| `DIST_TYPE`    | `0`, `1`      | Input distribution: uniform (`0`) or normal biased toward max positive with random sign (`1`) |
...
Random inputs are generated by `gen_a()` / `gen_b()` helper functions controlled by `MAX_VAL_A`, `MAX_VAL_B`, and `DIST_TYPE`. `MAX_VAL_*` is the maximum positive value (in the signed-positive range) directly — e.g. for 4-bit signed inputs the valid range is `1..7`, and for 8-bit signed inputs it is `1..127`. This permits arbitrary linear sweeps of the input dynamic range. When `DIST_TYPE=0` (uniform), values are drawn uniformly over the symmetric range `[-MAX_VAL_*, +MAX_VAL_*]`. When `DIST_TYPE=1` (normal), a Box-Muller transform produces a Gaussian distribution with mean `(7/8) × MAX_VAL_*` and σ = `(1/8) × MAX_VAL_*`, tightly concentrated near the maximum; samples are clamped to the absolute signed range of the full input width, then randomly negated to produce both positive and negative high-magnitude values. Corner cases (`verify_with_corner`) test `±MAX_VAL_*` directly.
...
| `MAX_VAL_A`    | sim and post-syn-sim, all top-levels        | `1`..`2^(IN_WIDTH_A-1)-1` | Max positive value for `a` test inputs; values drawn from `[-MAX_VAL_A, +MAX_VAL_A]` (default: `2^(IN_WIDTH_A-1)-1`) |
| `MAX_VAL_B`    | sim and post-syn-sim, non-alpha top-levels  | `1`..`2^(IN_WIDTH_B-1)-1` | Max positive value for `b` test inputs; values drawn from `[-MAX_VAL_B, +MAX_VAL_B]` (default: `2^(IN_WIDTH_B-1)-1`) |
| `DIST_TYPE`    | sim and post-syn-sim, all top-levels        | `0` (uniform), `1` (normal) | Random input distribution: uniform over `[-MAX_VAL_*, +MAX_VAL_*]` or normal tightly concentrated near max positive with random sign (Box-Muller, μ = (7/8)·MAX_VAL_*, σ = (1/8)·MAX_VAL_*) |
