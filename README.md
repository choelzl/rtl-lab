...
| `DYN_RANGE_A`  | `1`..`IN_WIDTH_A` | Active bit-width for `a` inputs; values are sign-extended into the full width (default: `IN_WIDTH_A`) |
| `DYN_RANGE_B`  | `1`..`IN_WIDTH_B` | Active bit-width for `b` inputs; values are sign-extended into the full width (default: `IN_WIDTH_B`; not available on alpha variants) |
| `DIST_TYPE`    | `0`, `1`      | Input distribution: uniform (`0`) or normal centered at unsigned midpoint (`1`) |
...
Random inputs are generated by `gen_a()` / `gen_b()` helper functions controlled by `DYN_RANGE_A`, `DYN_RANGE_B`, and `DIST_TYPE`. When `DIST_TYPE=0` (uniform), values are drawn uniformly over the `DYN_RANGE_*`-bit unsigned range and sign-extended into the full input width. When `DIST_TYPE=1` (normal), a Box-Muller transform produces a Gaussian distribution centered at the unsigned midpoint of the `DYN_RANGE_*`-bit range, clamped to that range before sign-extension. Corner cases (`verify_with_corner`) also respect `DYN_RANGE_*`: max-positive is `(2^(DYN_RANGE-1))-1` and min-negative is `-2^(DYN_RANGE-1)` sign-extended to the full width.
...
| `DYN_RANGE_A`  | sim and post-syn-sim, all top-levels        | `1`..`IN_WIDTH_A`       | Active bit-width for `a` test inputs; sign-extended to full width (default: `IN_WIDTH_A`)      |
| `DYN_RANGE_B`  | sim and post-syn-sim, non-alpha top-levels  | `1`..`IN_WIDTH_B`       | Active bit-width for `b` test inputs; sign-extended to full width (default: `IN_WIDTH_B`)      |
| `DIST_TYPE`    | sim and post-syn-sim, all top-levels        | `0` (uniform), `1` (normal) | Random input distribution: uniform over `DYN_RANGE_*` or normal (Box-Muller, σ = 2^(DYN_RANGE−2)) |
